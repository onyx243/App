...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('new-upload', () => {
      setMessage('New upload received');
      fetchUploads();
  ...
  return (
    <div>
      ...
      {message && <div className="bg-green-100 text-green-800 p-2 mb-2">{message}</div>}
        ...
        <tbody>
          {uploads.map((u) => (
            <tr key={u._id} className="border-t">
              ...
              <td className="p-2">
                {u.files.map((f) => {
                  const url = f.startsWith('http') ? f : `${import.meta.env.VITE_API_URL}/files/${encodeURIComponent(f)}`;
                  return (
                    <a
                      href={url}
                      key={f}
